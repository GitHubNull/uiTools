...
一个功能强大的基于Java Swing的图形界面工具，集成了多种开发者常用功能，包括字符串格式化、编解码、哈希计算、时间戳转换、二维码生成与解析、自动化输入、图片工具等。
...
### ⏰ 时间戳工具 (v1.6.3+)
- **获取当前时间**: 获取指定时区的当前时间，支持输出日期时间字符串或时间戳（10位/13位，支持前导补零）
- **时间戳转日期**: 将时间戳转换为指定时区和格式的日期时间字符串（自动识别10位/13位时间戳）
- **日期转时间戳**: 将日期时间字符串转换为时间戳（10位/13位，支持前导补零）
- **多时区支持**: 系统时区、UTC、GMT、EST、PST、CET、JST、AEST、IST、CST等11个时区
- **自定义格式**: 支持10种常用日期格式，支持自定义格式输入
- **面板化配置**: 独立配置面板，智能联动显示（如输出类型切换自动启用/禁用相关选项）
...
### 🖼️ 图片工具 (v1.6.5+)
- **生成空白图片**: 指定像素或寸数规格（1寸/2寸/小2寸/大1寸/护照/签证等），支持自定义背景色和输出格式
- **图片尺寸转换**: 转换为指定尺寸（自定义像素或预设寸数规格），支持保持宽高比
- **图片压缩**: JPEG有损压缩（可调压缩强度，值越大文件越小质量越低）和PNG无损压缩
- **压缩对比显示**: 勾选后可在日志区域查看压缩前后对比信息（原始大小、压缩后大小、减少率等）
- **图片输入支持**: 支持从文件选择或剪贴板粘贴
...
- **自动保存配置 (v1.6.4+)**: 可自定义自动保存目录，配置持久化存储
...
1. 选择"获取当前时间"操作
2. 配置面板选项：
   - **时区选择**: 从下拉框选择目标时区（系统时区、UTC、GMT、CST等）
   - **输出类型**: 选择"日期时间"或"时间戳"
   - **日期格式**: 当选择日期时间输出时，选择或自定义日期格式
   - **时间戳位数**: 当选择时间戳输出时，选择10位（秒）或13位（毫秒）
   - **前面补0**: 启用后在时间戳不足指定位数时前面补0
3. 点击"执行"按钮获取结果
...
2. 配置面板选项：
   - **时区选择**: 选择目标时区
   - **日期格式**: 选择或自定义输出日期格式
3. 在单行输入框中输入时间戳（10位或13位，自动识别）
4. 点击"执行"按钮

#### 日期转时间戳
1. 选择"日期转时间戳"操作
2. 配置面板选项：
   - **输入格式**: 选择或自定义日期格式（用于解析输入）
   - **输出位数**: 选择10位（秒）或13位（毫秒）
   - **前面补0**: 启用后在时间戳不足指定位数时前面补0
3. 在单行输入框中输入日期时间字符串
4. 点击"执行"按钮
...
│   ├── timestamp/                # 时间戳操作（3个）
│   │   ├── GetCurrentTimeOperation     # 获取当前时间
│   │   ├── TimestampToDatetimeOperation # 时间戳转日期
│   │   └── DatetimeToTimestampOperation # 日期转时间戳
│   ├── qrcode/                   # 二维码操作（2个）
...
└── ui/                    # 用户界面模块（refactored架构）
    ...
    │   ├── UIComponentRegistry.java      # UI组件注册表
    │   ├── ConfigPanelBuilder.java       # 配置面板构建器
    │   ├── InputPanelBuilder.java        # 输入面板构建器
    │   └── TimestampConfigPanelBuilder.java # 时间戳配置面板构建器
    ├── controller/          # 业务逻辑控制器
    │   ├── OperationValidator.java
    │   ├── OperationExecutor.java
    │   ├── OperationExecutionContext.java
    │   └── UIStateManager.java
    ├── handler/             # 事件处理模块
    │   ├── ClipboardHandler.java
    │   ├── EventHandler.java
    │   ├── KeyboardShortcutHandler.java
    │   └── TimestampConfigListener.java   # 时间戳配置联动监听器
    ├── image/               # 图片处理管理器
...
### v1.6.4 (2026-01-15)
- ✨ **新增**: 自动保存目录配置功能
  - 用户可自定义自动保存目录（点击"选择目录"按钮）
  - 配置通过 Java Preferences API 持久化存储，重启后保持
  - 当前配置目录实时显示（"默认: ~/uiTools_outputs" 或 "当前: {路径}"）
  - 配置目录无效时自动回退到默认目录
- 🎨 **新增**: SettingsManager 配置管理类（统一管理应用配置）
- 🎨 **新增**: 输出面板目录选择按钮和当前目录显示标签
- 🔧 **架构**: 配置持久化基础设施（支持未来扩展其他配置项）

### v1.6.3 (2026-01-15)
- ✨ **重构**: 时间戳功能从6个操作精简为3个核心操作
  - 获取当前时间（支持日期时间/时间戳输出，支持位数和前导补零配置）
  - 时间戳转日期（支持时区和自定义格式，自动识别10/13位时间戳）
  - 日期转时间戳（支持自定义格式输入，10/13位输出，支持前导补零）
- 🎨 **新增**: 时间戳配置面板系统（TimestampConfigPanelBuilder）
  - 获取当前时间配置面板（时区、输出类型、日期格式、时间戳位数、补0选项）
  - 时间戳转日期配置面板（时区、日期格式 + 单行输入框）
  - 日期转时间戳配置面板（输入格式、输出位数、补0选项 + 单行输入框）
- 🎨 **增强**: 单行输入框替代多行编辑器（时间戳转换操作）
- 🎨 **增强**: 智能UI联动（输出类型切换自动启用/禁用相关选项）
- 🎨 **修复**: 修复时间戳操作配置传递问题
- 🔧 **架构**: 新增TimestampConfigListener配置联动监听器
- 📦 **依赖**: 无新增依赖

### v1.6.2 (2026-01-15)
- 🎨 **重构**: 输入框区域动态布局管理
  - 根据操作类型动态切换输入面板（配置面板/图片面板/自动化面板等）
  - 移除不必要的输入框，提升用户体验
- 🎨 **新增**: InputPanelBuilder模块化输入面板构建器
- 🔧 **修复**: 修复图片操作验证逻辑

### v1.6.1 (2026-01-15)
- 🎨 **重构**: 操作分类两级分组（按功能类型分组）
- 🎨 **增强**: 移除顶部操作下拉框，改用树形分类导航
- 🔧 **重构**: 操作包结构重组为二级分组（encoding/formatting/hashing/timestamp/qrcode/automation/generator）
- 🎨 **新增**: 图片转Base64/Base32编码功能
- 🎨 **新增**: 随机密码生成器（支持字符类型、数量、长度配置）
